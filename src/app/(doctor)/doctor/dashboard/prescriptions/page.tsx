...
const PrescriptionsPage = () => {
		...
		const fetchPrescriptions = async () => {
			try {
				...
				const response = await fetch(
					`/api/prescriptions?doctorId=${userData._id}`
				);
				if (!response.ok) {
	...
	const filteredPrescriptions = prescriptions.filter((prescription) => {
		...
		if (activeTab === "draft")
			return matchesSearch && prescription.status === "draft";
		if (activeTab === "issued")
			return matchesSearch && prescription.status === "issued";
	...
	const renderPrescriptionCard = (prescription: Prescription) => {
		return (
			<Card
				...
				onClick={() =>
					router.push(
						`/doctor/dashboard/prescriptions/view/${prescription._id}`
					)
				}
			>
						...
						<div className="flex items-center">
							<Avatar className="h-10 w-10 mr-4">
								<AvatarImage
									...
									alt={prescription.patientId.fullName}
								/>
								<AvatarFallback>
									{prescription.patientId.fullName
										.split(" ")
					...
					<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
						...
						<div className="flex items-start">
							...
							<div>
								...
								<p className="text-sm text-gray-700">
									{format(new Date(prescription.updatedAt), "MMM d, yyyy")}
								</p>
							</div>
						</div>
					</div>
						...
						<Card>
							<CardContent className="py-10">
								<div className="flex flex-col items-center justify-center text-center">
									<FileText className="h-10 w-10 text-gray-400 mb-4" />
									<h3 className="text-lg font-medium text-gray-800 mb-2">
										No draft prescriptions
									</h3>
									<p className="text-gray-600 mb-4">
										You don&apos;t have any prescriptions saved as drafts
									</p>
								</div>
							</CardContent>
						</Card>
...
export default withRoleAccess(PrescriptionsPage, ["doctor"]);
